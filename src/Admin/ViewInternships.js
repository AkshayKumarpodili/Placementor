...
const ViewInternships = () => {
    ...
    useEffect(() => {
        ...
        const getCourses = async() => {
            ...
            console.log(id);
            setIsLoading(true);
            const q = query(collection(db,`TandPDb/${id}/internships`));
...
<div> <Loader/> </div>
        ...
        <>
                  ...
                  <>
                                    ...
                                    <p className="mt-[10px] font-epilogue font-bold text-[20px] text-black text-center"> These are internships done by {crename} </p>            
                                                {
                                                    courses.map((val,idx) =>(
